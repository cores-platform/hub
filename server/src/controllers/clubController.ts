  ...
  try {
    ...
    const clubInfo = {
      ...
      pendingMembers: userId && 
        (userMembershipStatus === 'owner' || userMembershipStatus === 'admin') 
        ? club.members
...
export const leaveClub = async (req: Request, res: Response) => {
  try {
    ...
    if (club.owner.toString() === userId) {
      res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: '동아리 소유자는 탈퇴할 수 없습니다. 먼저 소유권을 이전하세요.' });
      return;
    }
    ...
    res.status(StatusCodes.OK).json({ message: '동아리에서 탈퇴했습니다.' });
  } catch (error) {
